...
There is no Phase 3 ddG accuracy result: no head was trained, no normalization fitted, and no held-out test feature extraction/evaluation occurred. Phase 4 remains unstarted. `docs/` is locally Git-excluded; README and small aggregate artifacts also carry the reproduction/completion evidence.
